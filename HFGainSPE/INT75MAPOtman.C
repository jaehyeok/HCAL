...
void INT75MAPOtman(int runnum1=3711,int runnum2=0)
{

  //TString etaphifilename = TString::Format("EtaPhiMapRun%dFiveSigma.root",runnum1);
  TString etaphifilename = TString::Format("EtaPhiMapRun%dThreeSigma.root",runnum1);
  TFile *fileetaphimap = new TFile(etaphifilename.Data(),"recreate");
  ...
  for (int run=0; run<numofruns; run++){
	...
	for(int idepth=0; idepth<2; idepth++){printf("idepth %d\n",idepth);
	  for(int ieta=29; ieta<42; ieta++){printf("ieta %d\n",ieta);
		...
		h[idepth][ieta-29][iphi] ->GetXaxis()->SetRangeUser(parm[1]+3*parm[2],10000);
  ...
  cout<<"ev out of five sigma: "<<ped_ev_sigma->GetBinContent(33-28,8/2+1*36+1)<<endl;
  fileetaphimap->cd();
  ped_ev_sigma->Write();
  fileetaphimap->cd();
  fileetaphimap->Close();
